...
function generateCardsForMultiplicationEasy() {
  const pairs = [
    [3, 8],
    [4, 6],
    [2, 12],
  ...
  return { target: 24, cards };
}

export default function EasyMultiplication() {
  ...
  useEffect(() => {
    if (mode === 'multiplication' && difficulty === 'easy') {
      const { target, cards } = generateCardsForMultiplicationEasy();
      setTarget(target);
      setCards(cards);
    }
  }, [mode, difficulty]);
