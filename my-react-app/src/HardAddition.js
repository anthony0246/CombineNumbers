...
function generateCardsForAdditionHard() {
  const pairs = [
    [22, 18],
    [23, 17],
    [24, 16],
  ...
  return { target: 40, cards };
}

export default function HardAddition() {
  ...
  useEffect(() => {
    if (mode === 'addition' && difficulty === 'hard') {
      const { target, cards } = generateCardsForAdditionHard();
      setTarget(target);
      setCards(cards);
    }
  }, [mode, difficulty]);
