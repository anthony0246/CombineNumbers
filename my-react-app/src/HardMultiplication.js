...
function generateCardsForMultiplicationHard() {
  const pairs = [
    [12, 12],
    [18, 8],
    [24, 6],
  ...
  return { target: 144, cards };
}

export default function HardMultiplication() {
  ...
  useEffect(() => {
    if (mode === 'multiplication' && difficulty === 'hard') {
      const { target, cards } = generateCardsForMultiplicationHard();
      setTarget(target);
      setCards(cards);
    }
  }, [mode, difficulty]);
  ...
  return (
    <div style={styles.page}>
      <Container className="text-center py-5">
        <h2 style={styles.title}>Multiply the Numbers</h2>
        <p style={styles.subtitle}>
          Try to match cards whose product equals{' '}
