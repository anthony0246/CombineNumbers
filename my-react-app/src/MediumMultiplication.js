...
function generateCardsForMultiplicationMedium() {
  // 8 pairs whose product equals 48 (target)
  const pairs = [
    [1, 120],
    [2, 60],
    [3, 40],
  ...
  // flatten pairs and create cards
  ...
  return { target: 120, cards };
}

export default function MediumMultiplication() {
  ...
  useEffect(() => {
    if (mode === 'multiplication' && difficulty === 'medium') {
      const { target, cards } = generateCardsForMultiplicationMedium();
      setTarget(target);
      setCards(cards);
    }
  }, [mode, difficulty]);
  ...
  return (
    <div style={styles.page}>
      <Container className="text-center py-5">
        <h2 style={styles.title}>Multiply Numbers</h2>
        <p style={styles.subtitle}>
          Try to match cards that multiply to{' '}
            ...
            return (
              <Col xs={3} key={card.id} style={styles.col}>
                ...
                >
                  ...
                  >
                    <div style={{ ...styles.cardFace, ...styles.cardFront }}>?</div>
                    <div style={{ ...styles.cardFace, ...styles.cardBack }}>{card.value}</div>
                  </div>
                </div>
              </Col>
            );
